...
{
	printMap();//打印棋盘
	while (1) {                                																					 //这里再做成一个函数,分开用, 
		printCHESS();//打印棋子
		int result = getUserChoose();
		if (result == -1)return;
		int c = result / 100;																									//res两用
		int d = result % 100;
		result = fun1(c, d);
		printCHESS();//打印棋子
		if (result == 0) {
...
int fun1(int x, int y)
{
	...
	for (m = 0; m < MAX_LEN; m++)	//判断胜负
	{
		for (n = 0; n < MAX_LEN; n++)              																							
		{
			if (map[m][n] == 1 && map[m][n + 1] == 1 && map[m][n + 2] == 1 && map[m][n + 3] == 1 && map[m][n + 4] == 1)//竖线
				return 2;
			if (map[m][n] == 1 && map[m + 1][n] == 1 && map[m + 2][n] == 1 && map[m + 3][n] == 1 && map[m + 4][n] == 1)//横线
				return 2;
			if (map[m][n] == 1 && map[m + 1][n + 1] == 1 && map[m + 2][n + 2] == 1 && map[m + 3][n + 3] == 1 && map[m + 4][n + 4] == 1)//反斜线
				return 2;
			if (n > 3 && map[m][n] == 1 && map[m + 1][n - 1] == 1 && map[m + 2][n - 2] == 1 && map[m + 3][n - 3] == 1 && map[m + 4][n - 4] == 1)//斜线 >3才有5
				return 2;
